...
const serviceData = {
  ...
  'dashcams': {
    ...
    description: 'A dashcam in your car is a reliable way to record everything that happens on road. It gives you clear video proof in case of accidents, making it drive safe, and helps you stay out of trouble. As long as the wiring is clean and the setup is right, it works perfectly and doesn\'t affect the interior of your car.',
    features: [
...
function ImageDetail() {
  const { serviceId, imageIndex } = useParams()
  ...
  if (!service || !service.gallery || imageIndexNum >= service.gallery.length) {
    return (
      <div className="image-detail-page">
        <div className="image-not-found">
          <h1>Image Not Found</h1>
          <p>The image you're looking for doesn't exist.</p>
          <Link to={`/services/${serviceId}`} className="back-to-service">Back to Service</Link>
        </div>
      </div>
    )
  }

  const currentImage = service.gallery[imageIndexNum]

  return (
    <div className={`image-detail-page ${isLoaded ? 'loaded' : ''}`}>
      {/* Hero Section with Image */}
      <section className="image-hero">
        <div className="image-container">
          <img src={currentImage} alt={`${service.title} - Image ${imageIndexNum + 1}`} className="detail-image" />
          <div className="image-overlay" />
        </div>
        <div className="image-content">
          <div className="image-breadcrumb">
            <Link to="/services">Services</Link>
            <span> / </span>
            <Link to={`/services/${serviceId}`}>{service.title}</Link>
            <span> / </span>
            <span>Image {imageIndexNum + 1}</span>
          </div>
          <h1 className="image-title">{service.title}</h1>
          <p className="image-tagline">{service.tagline}</p>
        </div>
      </section>
      ...
      <section className="image-content">
        <div className="container">
          ...
          <div className="image-navigation">
            <Link to={`/services/${serviceId}`} className="nav-btn">
              Back to Service
            </Link>
            {imageIndexNum > 0 && (
              <Link to={`/services/${serviceId}/image/${imageIndexNum - 1}`} className="nav-btn">
                ← Previous Image
              </Link>
            )}
            {imageIndexNum < service.gallery.length - 1 && (
              <Link to={`/services/${serviceId}/image/${imageIndexNum + 1}`} className="nav-btn">
                Next Image →
              </Link>
            )}
