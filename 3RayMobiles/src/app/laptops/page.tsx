...
const types = ["Business", "Gaming", "Student"] as const;

export default function LaptopsPage() {
  const all = getProductsByCategory("laptops");
  ...
  return (
    <>
      <PageHeader
        eyebrow="Laptops"
        title="Notebooks for every workflow"
        description="From thin-and-light productivity machines to RGB-fueled gaming rigs."
      />
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        ...
        <div className="flex items-center justify-between mb-4 lg:hidden">
          ...
          <p className="text-sm text-muted-foreground">
            <span className="text-foreground font-medium">{filtered.length}</span> laptops
          </p>
        </div>
