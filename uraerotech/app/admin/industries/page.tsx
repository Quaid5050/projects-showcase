...
const emptyForm = {
  name: '', slug: '', description: '', content: '', icon: '✈️', featuresText: '', isActive: true
}
...
export default function AdminIndustriesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editing ? `/api/admin/industries/${editing.id}` : '/api/admin/industries'
      const method = editing ? 'PATCH' : 'POST'
      const res = await fetch(url, {
        method,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...formData, image, images, features })
      })
      if (res.ok) {
        setShowModal(false)
        resetForm()
        fetchIndustries()
        showSuccess(editing ? 'Industry updated!' : 'Industry created!')
      } else {
        showError('Failed to save industry')
      }
    } catch { showError('Failed to save industry') }
    finally { setUploading(false) }
  }

  const handleEdit = (ind: Industry) => {
    setEditing(ind)
    setFormData({ name: ind.name, slug: ind.slug, description: ind.description, content: ind.content, icon: ind.icon, featuresText: ind.features.join('\n'), isActive: ind.isActive })
    setMainImagePreview(ind.image || '')
    setMainImageFile(null)
    setGalleryFiles([])
    setGalleryPreviews([])
    setExistingGallery(ind.images || [])
    setShowModal(true)
  }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) { fetchIndustries(); showSuccess('Industry deleted!') }
      else showError('Failed to delete industry')
    } catch { showError('Failed to delete industry') }
    ...
    <div className="min-h-screen py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-4xl font-bold text-white">Manage Industries</h1>
            <p className="text-white/60 mt-2">Add, edit, or remove industries</p>
          </div>
          <div className="flex gap-4">
            <button onClick={openAdd} className="bg-gradient-aviation text-white px-6 py-3 rounded-lg hover:shadow-glow-blue transition">+ Add Industry</button>
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-xl overflow-hidden">
          <table className="w-full">
            <thead className="bg-white/5">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-white/50 uppercase">Industry</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-white/50 uppercase">Icon</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-white/50 uppercase">Features</th>
            ...
            <tbody className="divide-y divide-white/10">
              {industries.map((ind) => (
                <tr key={ind.id} className="hover:bg-white/5">
                  <td className="px-6 py-4">
                    <div className="font-semibold text-white">{ind.name}</div>
                    <div className="text-sm text-white/50">{ind.slug}</div>
                  </td>
                  <td className="px-6 py-4 text-2xl">{ind.icon}</td>
                  <td className="px-6 py-4 text-sm text-white/60">{ind.features.length} features</td>
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-full text-xs ${ind.isActive ? 'bg-green-500/20 text-green-400' : 'bg-white/10 text-white/50'}`}>
                      {ind.isActive ? 'Active' : 'Inactive'}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex gap-2">
                      <button onClick={() => handleEdit(ind)} className="text-blue-400 hover:text-blue-300 font-medium text-sm">Edit</button>
                      <button onClick={() => setDeleteConfirm({ show: true, id: ind.id, name: ind.name })} className="text-red-400 hover:text-red-300 font-medium text-sm">Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900/95 border border-white/10 rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-8">
            <h2 className="text-2xl font-bold text-white mb-6">{editing ? 'Edit Industry' : 'Add New Industry'}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-white/70 mb-2">Features (one per line)</label>
                <textarea rows={4} value={formData.featuresText}
                  onChange={(e) => setFormData({ ...formData, featuresText: e.target.value })}
                  placeholder="FAA certified repairs&#10;24/7 emergency response&#10;Fleet-wide support"
                  className="w-full px-4 py-2 bg-white/5 border border-white/10 text-white placeholder-white/30 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" />
              </div>
              ...
              <div className="flex gap-4 pt-4">
                <button type="submit" disabled={uploading}
                  ...
                  {uploading ? 'Uploading...' : editing ? 'Update Industry' : 'Add Industry'}
                </button>
      ...
      {deleteConfirm.show && (
        <ConfirmDialog
          ...
          disclaimer="All quote requests linked to this industry will also be permanently deleted."
