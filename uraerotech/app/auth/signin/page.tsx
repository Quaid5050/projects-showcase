...
function SignInForm() {
  ...
  useEffect(() => {
    if (searchParams.get('registered') === 'true') showSuccess('Account created successfully! Please sign in.')
  }, [searchParams])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        showError(result.error.includes('role') ? 'Invalid role selected.' : 'Invalid email or password')
      } else {
      ...
      <div className="max-w-md w-full">
        <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-8">
          <div className="text-center mb-8">
            ...
            <p className="mt-2 text-white/60">Welcome back to UR Aerotech</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="flex gap-2 p-1 bg-white/5 border border-white/10 rounded-lg">
                {[['B2C', '👤 Individual'], ['B2B', '🏢 Business'], ['ADMIN', '🔐 Admin']].map(([role, label]) => (
