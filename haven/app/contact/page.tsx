  ...
  return(
    <main style={{background:"#080808",paddingTop:"100px"}}>
      ...
      <section style={{position:"relative",height:"55vh",display:"flex",alignItems:"center",overflow:"hidden"}}>
        ...
        <img src={`${UNSPLASH}/photo-1603386329225-868f9b1ee6c9?w=1400&q=80`} alt="Contact" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",filter:"brightness(0.2) saturate(1.1)"}} onError={e=>{(e.target as HTMLImageElement).style.display="none"}}/>
        ...
        <div style={{position:"relative",padding:"0 60px",zIndex:2,maxWidth:"700px"}}>
          ...
          <p style={{fontSize:"18px",color:"rgba(240,240,240,0.88)",lineHeight:1.85,maxWidth:"460px"}}>Same-week slots available. Tell us about your vehicle and service needed and we will confirm within 24 hours.</p>
        </div>
      </section>
        ...
        <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:"60px",alignItems:"start"}} className="contact-grid">
            ...
            {submitted ? (
              <div style={{textAlign:"center",padding:"60px 0"}}>
                <div style={{width:"64px",height:"64px",border:"2px solid #e8001d",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 24px"}}>
                  <svg width="28" height="28" viewBox="0 0 28 28" fill="none" stroke="#e8001d" strokeWidth="2" strokeLinecap="round"><path d="M5 14 L11 20 L23 8"/></svg>
                </div>
                <h3 style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:"42px",color:"#f0f0f0",marginBottom:"12px"}}>REQUEST RECEIVED</h3>
                <p style={{fontSize:"17px",color:"rgba(240,240,240,0.88)",lineHeight:1.8}}>We will contact you within 24 hours to confirm your appointment.<br/>Check your email for a summary.</p>
              </div>
            ):(
              <form onSubmit={submit}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"20px",marginBottom:"20px"}} className="form-row">
                  {[{k:"name",l:"Full Name *",ph:"John Smith",type:"text"},{k:"email",l:"Email Address *",ph:"john@example.com",type:"email"},{k:"phone",l:"Phone Number",ph:"(555) 000-0000",type:"tel"},{k:"vehicle",l:"Your Vehicle",ph:"2023 BMW M3 Competition",type:"text"}].map(f=>(
                    <div key={f.k}>
                      ...
                      <input type={f.type} placeholder={f.ph} required={f.k==="name"||f.k==="email"} value={(form as Record<string,string>)[f.k]} onChange={e=>set(f.k,e.target.value)}
                        style={{width:"100%",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",padding:"14px 16px",color:"#f0f0f0",fontFamily:"'Rajdhani',sans-serif",fontSize:"16px",outline:"none",transition:"border-color .3s"}}
                ...
                <div style={{marginBottom:"20px"}}>
                  <label style={{display:"block",fontFamily:"'Orbitron',sans-serif",fontSize:"12px",letterSpacing:"3px",color:"rgba(240,240,240,0.82)",textTransform:"uppercase",marginBottom:"8px"}}>Service Required *</label>
                  <select value={form.service} onChange={e=>set("service",e.target.value)} required
                    style={{width:"100%",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",padding:"14px 16px",color:form.service?"#f0f0f0":"rgba(240,240,240,0.72)",fontFamily:"'Rajdhani',sans-serif",fontSize:"16px",outline:"none",transition:"border-color .3s",appearance:"none"}}
                    onFocus={e=>(e.target.style.borderColor="#e8001d")}
                    onBlur={e=>(e.target.style.borderColor="rgba(255,255,255,0.1)")}>
                ...
                <div style={{marginBottom:"32px"}}>
                  <label style={{display:"block",fontFamily:"'Orbitron',sans-serif",fontSize:"12px",letterSpacing:"3px",color:"rgba(240,240,240,0.82)",textTransform:"uppercase",marginBottom:"8px"}}>Additional Notes</label>
                  <textarea value={form.message} onChange={e=>set("message",e.target.value)} rows={4} placeholder="Tell us more about your requirements, preferred dates, or any special considerations..."
                    style={{width:"100%",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",padding:"14px 16px",color:"#f0f0f0",fontFamily:"'Rajdhani',sans-serif",fontSize:"16px",outline:"none",resize:"vertical",transition:"border-color .3s"}}
                    onFocus={e=>(e.target.style.borderColor="#e8001d")}
                    onBlur={e=>(e.target.style.borderColor="rgba(255,255,255,0.1)")}/>
                </div>
                <button type="submit" className="clip-btn" data-hover style={{width:"100%",background:"#e8001d",color:"white",border:"none",padding:"16px",fontFamily:"'Orbitron',sans-serif",fontSize:"14px",fontWeight:700,letterSpacing:"4px",textTransform:"uppercase",cursor:"pointer",transition:"all .3s"}}
                onMouseEnter={e=>{e.currentTarget.style.background="#ff0025";e.currentTarget.style.boxShadow="0 0 40px rgba(232,0,29,0.6)"}}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:"2px"}}>
            ...
            <div style={{background:"#0d0d0d",border:"1px solid rgba(255,255,255,0.07)",overflow:"hidden",position:"relative",height:"220px"}}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={`${UNSPLASH}/photo-1449824913935-59a10b8d2000?w=600&q=80`} alt="Location" style={{width:"100%",height:"100%",objectFit:"cover",filter:"brightness(0.35) saturate(0.5)"}} onError={e=>{(e.target as HTMLImageElement).style.display="none"}}/>
      ...
      <style>{`@media(max-width:900px){.contact-grid{grid-template-columns:1fr!important}.form-row{grid-template-columns:1fr!important}section{padding-left:24px!important;padding-right:24px!important}}`}</style>
    </main>
  );
}
