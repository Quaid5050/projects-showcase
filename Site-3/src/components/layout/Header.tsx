    ...
    <>
      ...
      >
        <div className="container mx-auto px-6 md:px-10 flex items-center justify-between h-[88px]">
          ...
          {/* Logo — left */}
          <Link href="/" className="flex items-center shrink-0 z-50" onClick={() => setMobileMenuOpen(false)}>
            <img
              src="/images/logo.png"
      ...
      {/* Mobile Nav — rendered outside header so it truly covers full viewport */}
      <div
        className={cn(
          "fixed inset-0 z-[55] bg-slate-950 flex flex-col pt-28 px-8 md:hidden transition-transform duration-300 ease-in-out overflow-y-auto",
        ...
        <nav className="flex flex-col gap-6">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              href={link.href}
              className="text-2xl font-semibold text-slate-100 hover:text-emerald-400 transition-colors border-b border-slate-800 pb-4"
